    ...
    <div className="min-h-screen pb-24 px-4 pt-4 max-w-lg mx-auto">
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {items.map((item: any) => {
            const name = item.contacts
              ? `${item.contacts.first_name} ${item.contacts.last_name}`.trim()
              : "Unknown";
            return (
              <button
                key={item.id}
                onClick={() => navigate(`/followup/${item.id}`)}
                className="w-full text-left bg-card rounded-lg border border-border p-4 flex items-center justify-between active:scale-[0.98] transition-transform"
              >
                <div className="min-w-0">
                  <h3 className="font-semibold text-foreground truncate">{name}</h3>
                  {item.contacts?.company && (
                    <p className="text-sm text-muted-foreground">{item.contacts.company}</p>
                  )}
                </div>
